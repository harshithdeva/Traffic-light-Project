...
if __name__ == '__main__':
    ...
    sumo_env = SumoEnvironment(
        ...
        # out_csv_name='results/dqnhighhigh_model/dqn_test',
        out_csv_name='results/dqnhighhigh_fixed/dqn_test',
        single_agent=True,
        use_gui=False,
        num_seconds=15000,
        min_green=5)
    ...
    model = DQN("MlpPolicy",
                ...
                verbose=1,
                learning_rate=0.001)
    model.load('DQN/DQNHighLogs/best_model')
...
# Comment from here
    ...
    print('\nTime:', traffic_signal[0].get_accumulated_waiting_time_per_lane())
    print('Fixed Time', sum(rewards_fixed),
          '\n', 'Model Time', sum(rewards_model))
    print(sum(rewards_model) / sum(rewards_fixed))
